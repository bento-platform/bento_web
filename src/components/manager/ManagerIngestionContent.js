...
const ManagerIngestionContent = ({
    ...
    projectsByID,
    tablesByServiceID,
    isSubmittingIngestionRun,
    workflows,
    submitIngestionWorkflowRun,
}) => {
    const [step, setStep] = useState(
        location?.state?.step || STEP_WORKFLOW_SELECTION
    );
    const [selectedTable, setSelectedTable] = useState(
        location?.state?.selectedTable || null
    );
    const [selectedWorkflow, setSelectedWorkflow] = useState(
        location?.state?.selectedWorkflow || null
    );
    const [initialInputValues, setInitialInputValues] = useState(
        location?.state?.initialInputValues || {}
    );
    ...
    const handleRunIngestion = (history) => {
            ...
            case STEP_WORKFLOW_SELECTION: {
                const filteredWorkflows = workflows
                    .filter(
                        (w) =>
                            w.data_type ===
                            (selectedTable ? selectedTable.split(":")[1] : null)
                    )
                    .map((w) => (
                        <WorkflowListItem
                            key={w.id}
                    ...
                    >
                        ...
                        <Form.Item wrapperCol={FORM_BUTTON_COL}>
                            ...
                            <Button
                                ...
                                onClick={() => handleRunIngestion(history)}
                            >
                                Run Ingestion
                            </Button>
                        </Form.Item>
...
ManagerIngestionContent.propTypes = {
    ...
    submitIngestionWorkflowRun: PropTypes.func,
};
...
//                 this.initialState.step,
//             selectedTable:
//                 (this.props.location.state || {}).selectedTable ||
//                 this.initialState.selectedTable,
//             selectedWorkflow:
//                 (this.props.location.state || {}).selectedWorkflow ||
//                 this.initialState.selectedWorkflow,
//             initialInputValues:
//                 (this.props.location.state || {}).initialInputValues ||
...
//         const serviceInfo =
//             this.props.servicesByID[this.state.selectedWorkflow.serviceID];
//         const tableID = this.state.selectedTable.split(":")[2];

//         this.props.submitIngestionWorkflowRun(
//             serviceInfo,
//             tableID,
...
//         const getTableName = (serviceID, tableID) =>
//             (
//                 ((this.props.tablesByServiceID[serviceID] || {}).tablesByID ||
//                     {})[tableID] || {}
//             ).name;
...
//                         onChange={(formValues) =>
//                             this.setState({ inputFormFields: formValues })
//                         }
...
//                     this.state.selectedTable.split(":");
//                 const projectTitle =
//                     (this.props.projectsByID[projectID] || { title: null })
//                         .title || null;
//                 const tableName = getTableName(
//                     this.state.selectedWorkflow.serviceID,
//                     tableID
//                 );
...
//                                 ]}
//                                 rowKey="id"
//                                 dataSource={this.state.selectedWorkflow.inputs.map(
//                                     (i) => ({
//                                         id: i.id,
//                                         value: this.state.inputs[i.id],
//                                     })
//                                 )}
//                             />
//                         </Form.Item>
...
//                                 onClick={() =>
//                                     this.handleRunIngestion(this.props.history)
//                                 }
//                             >
//                                 Run Ingestion
//                             </Button>
