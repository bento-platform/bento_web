...
class ManagerProjectDatasetContent extends Component {
    render() {
        const projectMenuItems = this.props.projects.map((project) => ({
            url: withBasePath(
                `admin/data/manager/projects/${project.identifier}`
            ),
            text: project.title,
        }));
                    ...
                    projectMenuItems.length === 0 ? (
                        <Layout.Content style={LAYOUT_CONTENT_STYLE}>
                            ...
                            >
                                ...
                                >
                                    To create datasets and ingest data, you have
                                    to create a CHORD project first. CHORD
                                    projects have a name and description, and
                                    let you group related datasets together. You
                                    can then specify project-wide consent codes
                                    and data use restrictions to control data
                                    access.
                                </Typography.Paragraph>
                                <Button
                                    type="primary"
                                ...
                                {projectMenuItems.length > 0 ? (
                                    <Switch>
                                        ...
                                        <Redirect
                                            ...
                                            to={withBasePath(
                                                `admin/data/manager/projects/${this.props.projects[0].identifier}`
                                            )}
                                        />
                                    </Switch>
                                ) : this.props.loadingAuthDependentData ? (
