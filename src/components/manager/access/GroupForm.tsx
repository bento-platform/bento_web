...
const MembershipInput = ({ value, onChange, ...rest }: MembershipInputProps) => {
  ...
  const onChangeExpr = useCallback<InputChangeEventHandler>(
    (e) => {
        ...
        {membershipType === "list" ? (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "row", gap: 16, marginTop: 16 }}>
              ...
              <Button icon={<PlusOutlined />} onClick={onAddMember}>
                Add
              </Button>
            </div>
          </>
...
const GroupForm = ({ form }: GroupFormProps) => (
  <Form form={form} layout="vertical">
    ...
    <Form.Item name="membership" label="Membership" initialValue={{ members: [] }} rules={MEMBERSHIP_VALIDATOR_RULES}>
      <MembershipInput />
    </Form.Item>
