...
const ExpiryTimestamp = ({ expiry }: { expiry?: string }) => {
  ...
  const spanStyle = useMemo((): CSSProperties => (expired ? { color: COLOR_ANTD_RED_6 } : {}), [expiry, expired]);
  ...
  return (
    <Popover
      content={
        <span>
          UTC timestamp: <MonospaceText>{expiry}</MonospaceText>
        </span>
      }
    >
      <span style={spanStyle}>
        {new Date(Date.parse(expiry)).toLocaleString()} {expired ? <em>(EXPIRED)</em> : ""}
      </span>
    </Popover>
  );
};
