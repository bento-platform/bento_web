...
class RunLog extends Component {
    componentDidMount() {
        if (!this.props.isFetchingRuns) {
            this.props.fetchRunLogStreamsIfPossibleAndNeeded(
                this.props.run.run_id
            );
        }
    }
    ...
    render() {
        ...
        const stdout =
            (this.props.runLogStreams[run.run_id] || {}).stdout || null;
        const stderr =
            (this.props.runLogStreams[run.run_id] || {}).stderr || null;
        return (
            <Descriptions bordered style={{ overflow: "auto" }}>
                ...
                <Descriptions.Item label="Exit Code" span={1}>
                    {details.run_log.exit_code === null
                        ? "N/A"
                        : details.run_log.exit_code}
                </Descriptions.Item>
                <Descriptions.Item
                    label={
                        <span style={{ fontFamily: "monospace" }}>stdout</span>
                    }
                    span={3}
                >
                    {stdout !== null ? (
                ...
                <Descriptions.Item
                    label={
                        <span style={{ fontFamily: "monospace" }}>stderr</span>
                    }
                    span={3}
                >
                    {stderr !== null ? (
...
RunLog.propTypes = {
    ...
    runLogStreams: PropTypes.objectOf(
        PropTypes.objectOf(
            PropTypes.shape({
