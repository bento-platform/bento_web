...
const Run = ({ tab, run, onBack, onChangeTab }) => {
    run = run ?? {};
    tab = tab ?? "request";
    ...
    return (
        <>
            <PageHeader
                ...
                tags={
                    <Tag color={RUN_STATE_TAG_COLORS[run.state]}>
                        {run.state}
                    </Tag>
                }
                footer={
                    <Tabs activeKey={tab} onChange={onChangeTab || nop}>
            ...
            >
                <Row type="flex">
                    <Statistic
                        title="Started"
                        value={
                            renderDate(run.details.run_log.start_time) || "N/A"
                        }
                    />
                    <Statistic
                        title="Ended"
                        value={
                            renderDate(run.details.run_log.end_time) || "N/A"
                        }
