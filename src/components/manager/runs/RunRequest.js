...
const RunRequest = ({ run, tablesByServiceID }) => {
    ...
    const tableName = (
        ((tablesByServiceID[serviceID] || {}).tablesByID || {})[tableID] || {}
    ).name;
    ...
    return (
        <Descriptions bordered>
            <Descriptions.Item label="Table" span={3}>
                <Tag>{tableDataType}</Tag>{" "}
                {tableName ? (
                    <>
                        {tableName} ({idFragment})
                    </>
                ) : (
                    idFragment
                )}
            ...
            <Descriptions.Item label="Workflow URL">
                <a
                    href={details.request.workflow_url}
                    target="_blank"
            ...
            <Descriptions.Item label="Workflow" span={3}>
                <List itemLayout="vertical">
                    <WorkflowListItem
                        workflow={details.request.tags.workflow_metadata}
                    />
                </List>
            </Descriptions.Item>
            <Descriptions.Item label="Tags">
                <pre style={{ margin: 0 }}>
                    {JSON.stringify(details.request.tags, null, 4)}
                </pre>
            </Descriptions.Item>
        </Descriptions>
    );
