...
class ServiceLogs extends Component {
    render() {
        ...
        const loading =
            ...
            this.props.serviceLogs.isFetching;

        const logList = Object.entries((logs || {}).logs || {}).map(
            (l) => l[0]
        );
                ...
                >
                    {loading ? (
                        ...
                        <Switch>
                            <Route
                                path={withBasePath(
                                    "admin/services/:artifact/logs/:log"
                                )}
                                component={ServiceLog}
                            />
                            <Redirect
                                from={withBasePath(
                                    `admin/services/${artifact}/logs`
                                )}
                                to={withBasePath(
                                    `admin/services/${artifact}/logs/${logList[0]}`
                                )}
                            />
                        </Switch>
                    ) : (
