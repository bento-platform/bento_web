...
const VariantSearchHeader = ({ dataType, addVariantSearchValues }) => {
    ...
    const genotypeSchema =
        dataType.schema?.properties?.calls?.items?.properties?.genotype_type;
    ...
    return (
        <>
            ...
            >
                <Select onChange={handleAssemblyIdChange}>
                    {assemblySchema.enum.map((v) => (
            ...
            <Form.Item
                ...
                label={"Gene / position"}
                help={
                    "Enter gene name (eg \"BRCA1\") or position (\"chr17:41195311-41278381\")"
                }
            >
                <LocusSearch
                    assemblyId={lookupAssemblyId}
            ...
            >
                <Select onChange={handleGenotypeChange}>
                    {genotypeSchema.enum.map((v) => (
                        <Select.Option key={v} value={v}>
                            {v}
                        </Select.Option>
                    ))}
