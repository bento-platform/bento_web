...
class DiscoveryQueryBuilder extends Component {
    constructor(props) {
        ...
        this.handleAddDataTypeQueryForm =
            this.handleAddDataTypeQueryForm.bind(this);
        this.handleTabsEdit = this.handleTabsEdit.bind(this);
        this.handleVariantHiddenFieldChange =
            this.handleVariantHiddenFieldChange.bind(this);

        this.forms = {};
    }

    componentDidMount() {
        (this.props.requiredDataTypes ?? []).forEach((dt) =>
            this.props.addDataTypeQueryForm(dt)
        );
        ...
        if (this.props.autoQuery?.isAutoQuery) {
            ...
            (async () => {
                ...
                // Set type of query
                await this.handleAddDataTypeQueryForm({
                    key: this.props.autoQuery.autoQueryType,
                });

                // Set term
                const dataType =
                    this.props.dataTypesByID[
                        this.props.autoQuery.autoQueryType
                    ];
                const fields = {
                    keys: {
                        value: [0],
                    ...
                    conditions: [
                        {
                            ...
                            value: {
                                ...
                                fieldSchema: getFieldSchema(
                                    dataType.schema,
                                    this.props.autoQuery.autoQueryField
                                ),
                                negated: false,
                                operation: OP_EQUALS,
                                searchValue:
                                    this.props.autoQuery.autoQueryValue,
                            },
                        },
                    ],
                };
    ...
    handleVariantHiddenFieldChange(fields) {
        this.props.updateDataTypeQueryForm(
            this.props.dataTypesByID["variant"],
            fields
        );
    }
    ...
    handleAddDataTypeQueryForm(e) {
        ...
        this.props.addDataTypeQueryForm(
            this.props.dataTypesByID[keySplit[keySplit.length - 1]]
        );
    }
    ...
    render() {
        ...
        const dataTypeTabPanes = this.props.dataTypeForms.map((d) => (
            <Tabs.TabPane
                ...
                key={d.dataType.id}
                closable={
                    !(this.props.requiredDataTypes ?? []).includes(
                        d.dataType.id
                    )
                }
            >
        ...
        const addConditionsOnDataType = (
            buttonProps = { style: { float: "right" } }
        ) => (
            <Dropdown
                overlay={dataTypeMenu}
                disabled={
                    this.props.dataTypesLoading || this.props.searchLoading
                }
            >
                <Button {...buttonProps}>
                    Add Conditions on Data Type <Icon type="down" />
                </Button>
            </Dropdown>
        );
            ...
            <Card style={{ marginBottom: "1.5em" }}>
                ...
                <Typography.Title level={3} style={{ marginBottom: "1.5rem" }}>
                    ...
                    <Button
                        ...
                        onClick={this.handleSchemasToggle}
                    >
                        <Icon type="table" /> Explore Data Types
                    </Button>
                    <Button
                        style={{ float: "right", marginRight: "1em" }}
                        onClick={() => {
                                ...
                                content: (
                                    <>
                                        <Typography.Paragraph>
                                            CHORD defines multiple queryable
                                            data types for researchers to take
                                            advantage of to standardize their
                                            datasets and make them discoverable.
                                            Each of these data types is defined
                                            by a <strong>schema</strong>, which
                                            specifies all the components of a
                                            single object in a table of a given
                                            data type. Some of the fields of
                                            these objects are directly
                                            queryable, while others are not;
                                            this is determined in part by the
                                            sensitivity of the field.
                                        </Typography.Paragraph>
                                        <Typography.Paragraph>
                                            Data types and their schemas can be{" "}
                                            <a onClick={destroyHelpModal}>
                                                explored
                                            </a>{" "}
                                            in both a tree and a searchable
                                            table structure.
                                        </Typography.Paragraph>
                                        <Typography.Paragraph>
                                            If two or more data types are
                                            queried at the same time, the
                                            federated search system will look
                                            for datasets that have linked data
                                            objects matching both criteria. This
                                            first requires that researchers have
                                            correctly set up their datasets to
                                            link e.g. patients with their
                                            corresponding genomic variants.
                                        </Typography.Paragraph>
                                    </>
                                ),
                                maskClosable: true,
                ...
                {this.props.dataTypeForms.length > 0 ? (
                    ...
                    <Empty
                        ...
                        description="No Data Types Added"
                    >
...
const mapStateToProps = (state) => ({
    ...
    dataTypesLoading:
        ...
        state.serviceDataTypes.isFetchingAll ||
        Object.keys(state.serviceDataTypes.dataTypesByServiceID).length === 0,
});

export default connect(mapStateToProps, { neutralizeAutoQueryPageTransition })(
