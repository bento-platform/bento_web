...
class DiscoverySearchCondition extends Component {
    static getDerivedStateFromProps(nextProps) {
        return "value" in nextProps ? { ...(nextProps.value ?? {}) } : null;
    }
    ...
    handleField = (value, key = "field", fieldSchemaKey = "fieldSchema") => {
        ...
        const change = {
            ...
            operation: fieldOperations.includes(this.state.operation)
    ...
    handleSearchValue = (e) => {
        this.handleChange({
            searchValue: getSchemaTypeTransformer(
                this.state.fieldSchema.type
            )[0](e.target.value),
        });
    };
    ...
    getSearchValue() {
        return getSchemaTypeTransformer(this.state.fieldSchema.type)[1](
    ...
    equalsOnly() {
        return (
            this.state.fieldSchema.search.operations.includes(OP_EQUALS) &&
            this.state.fieldSchema.search.operations.length === 1 &&
            (this.props.conditionType !== "join" ||
                (this.state.fieldSchema2.search.operations.includes(
                    OP_EQUALS
                ) &&
        ...
        if (
            ...
            return (
                ...
                >
                    {(this.state.fieldSchema.type === "boolean"
                        ? BOOLEAN_OPTIONS
                        : this.state.fieldSchema.enum
                    ).map((v) => (
                        <Select.Option key={`_${v}`} value={v}>
                            {v}
                        </Select.Option>
                    ))}
                </Select>
    ...
    getInputStyle = (valueWidth, div = 1) => ({
        width: `calc(${100 / div}% - ${valueWidth / div}px)`,
    });
    ...
    render() {
        ...
        const canRemove = !(
            this.state.fieldSchema.search.hasOwnProperty("type") &&
            this.state.fieldSchema.search.type === "single" &&
            this.state.fieldSchema.search.required
        );

        const canNegate =
            conditionType === "join" || this.state.fieldSchema.search.canNegate;

        // Subtract 1 from different elements' widths due to -1 margin-left
        const valueWidth =
            (conditionType === "join" ? 0 : DATA_TYPE_FIELD_WIDTH) +
        ...
        const schemaTreeSelect = (fieldKey, fieldSchemaKey, schema, style) => (
            <SchemaTreeSelect
                ...
                value={{
                    selected: this.state[fieldKey],
                    schema: this.state[fieldSchemaKey],
                }}
        ...
        const operationOptions = this.state.fieldSchema.search.operations.map(
            (o) => <Select.Option key={o}>{OPERATION_TEXT[o]}</Select.Option>
        );

        return (
            <Input.Group compact>
                {schemaTreeSelect(
                    // LHS TODO: Redo base level name
                    "field",
                    "fieldSchema",
                    conditionType === "join"
                        ? this.props.joinedSchema
                        : this.state.dataType?.schema,
                    {
                        ...(conditionType === "join"
                ...
                {canNegate ? ( // Negation
                    <Select
                        style={{ width: `${NEGATION_WIDTH}px`, float: "left" }}
...
const mapStateToProps = (state) => ({
    joinedSchema: {
        ...
        properties: Object.fromEntries(
            Object.entries(state.serviceDataTypes.itemsByID).map(([k, v]) => [
                k,
                {
                    type: "array",
                    items: v.schema,
