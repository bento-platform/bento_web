...
const SearchSummaryModal = ({ searchResults, ...props }) => {
    ...
    // Individuals summary
    const numIndividualsBySex = Object.fromEntries(
        SEX_VALUES.map((v) => [v, 0])
    );
    ...
    searchFormattedResults.forEach((r) => {
        ...
        (r.phenotypic_features || []).forEach((pf) => {
            ...
            numPhenoFeatsByType[pf.type.label] =
                (numPhenoFeatsByType[pf.type.label] || 0) + 1;
        });

        (r.diseases || []).forEach((d) => {
            // TODO: Better ontology awareness - label vs id, translation, etc.
            numDiseasesByTerm[d.term.label] =
                (numDiseasesByTerm[d.term.label] || 0) + 1;
        });

        (r.biosamples || []).forEach((b) => {
            const tissueKey = (b.sampled_tissue || {}).label || "N/A";
            const histDiagKey = (b.histological_diagnosis || {}).label || "N/A";
            numSamplesByTissue[tissueKey] =
                (numSamplesByTissue[tissueKey] || 0) + 1;
            numSamplesByHistologicalDiagnosis[histDiagKey] =
                (numSamplesByHistologicalDiagnosis[histDiagKey] || 0) + 1;
        });

        (r.experiments || []).forEach((e) => {
            numExperimentsByType[e.experiment_type] =
                (numExperimentsByType[e.experiment_type] || 0) + 1;
        });
    });

    const sexPieChartData = mapNameValueFields(
        numIndividualsBySex,
        thresholdProportion
    );
    ...
    const phenotypicFeaturesData = mapNameValueFields(
        numPhenoFeatsByType,
        thresholdProportion
    );
    const diseasesData = mapNameValueFields(
        numDiseasesByTerm,
        thresholdProportion
    );
    const biosamplesByTissueData = mapNameValueFields(
        numSamplesByTissue,
        thresholdProportion
    );
    ...
    const experimentsByTypeData = mapNameValueFields(
        numExperimentsByType,
        thresholdProportion
    );
        ...
        >
            <Row gutter={16} style={{ display: "flex", flexWrap: "wrap" }}>
                <Col span={7}>
                    <Statistic
                        title="Individuals"
                        value={searchFormattedResults.length}
                    />
                </Col>
                <Col span={7}>
                    <Statistic
