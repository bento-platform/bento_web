...
const InnerTimeElement = ({ type, timeElement }) => {
  switch (type) {
    ...
    case "gestational_age":
      return (
        <span>
          <strong>Weeks:</strong> {timeElement.gestationalAge.weeks} <strong>Days:</strong>{" "}
    ...
    case "age_range":
      return (
        <span>
          <strong>Start:</strong> <>{timeElement.age_range.start.iso8601duration}</> <strong>End:</strong>{" "}
