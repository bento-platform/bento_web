...
class LinkedFieldSetForm extends Component {
    ...
    addField() {
        this.props.form.setFieldsValue({
            [FIELD_KEYS]: [
                ...this.props.form.getFieldValue(FIELD_KEYS),
                fieldKey++,
            ],
        });
    }
    ...
    removeField(key) {
        this.props.form.setFieldsValue({
            [FIELD_KEYS]: this.props.form
                .getFieldValue(FIELD_KEYS)
                .filter((k) => k !== key),
        });
    }
                ...
                .forEach(([dt, f], i) => {
                    ...
                    try {
                        this.props.form.getFieldDecorator(`fields[${i}]`, {
                            initialValue: {
                                selected,
                                schema: getFieldSchema(rootSchema, selected),
                            },
                        });
                    } catch (err) {
    ...
    rootSchema() {
        return {
            ...
            properties: Object.fromEntries(
                Object.entries(this.props.dataTypes).map(([k, v]) => [
                    k,
                    {
                        type: "array",
                        items: v.schema,
                ...
                <Input.Group compact={true}>
                    {getFieldDecorator(`fields[${k}]`, {
                        rules: [
                            {
                                required: true,
                                message: "Please specify a field",
                            },
                        ],
                    })(
                        <SchemaTreeSelect
...
export default Form.create({ name: "linked_field_set_form" })(
    LinkedFieldSetForm
);
