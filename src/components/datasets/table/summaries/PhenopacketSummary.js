...
const PhenopacketSummary = ({ summary }) => {
    const individualsBySex = Object.entries(
        summary.data_type_specific.individuals.sex
    )
        .filter((e) => e[1] > 0)
        .map(([x, y]) => ({ x, y }));
    const individualsByKaryotype = Object.entries(
        summary.data_type_specific.individuals.karyotypic_sex
    )
        .filter((e) => e[1] > 0)
        .map(([x, y]) => ({ x, y }));
        ...
        <>
            ...
            <Row gutter={16}>
                ...
                <Col span={8}>
                    <Statistic
                        title="Biosamples"
                        value={summary.data_type_specific.biosamples.count}
                    />
                </Col>
                <Col span={8}>
                    <Statistic
                        title="Individuals"
                        value={summary.data_type_specific.individuals.count}
                    />
                </Col>
            </Row>
            {individualsBySex.length > 0 &&
            individualsByKaryotype.length > 0 ? (
                    <>
                    <Divider />
