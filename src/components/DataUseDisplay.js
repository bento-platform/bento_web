...
const DataUseDisplay = ({ dataUse }) => {
    const primaryCode = dataUse.consent_code.primary_category.code;
    const uses = dataUse.data_use_requirements.map((u) => u.code) || [];
            ...
            <div>
                ...
                <Row gutter={10} type="flex">
                    <Col>
                        ...
                        <Popover
                            {...PRIMARY_CONSENT_CODE_INFO[primaryCode]}
                            overlayStyle={{ maxWidth: "576px" }}
                        >
                            <Tag color="blue" style={TAG_STYLING}>
                                {dataUse.consent_code.primary_category.code}
                            </Tag>
                        </Popover>
                    </Col>
                    <Col>
                        <div style={TAG_LABEL_STYLING}>Secondary</div>
                        {dataUse.consent_code.secondary_categories.length >
                        0 ? (
                                dataUse.consent_code.secondary_categories.map(
                                    (sc) => (
                                    <Popover
                                        {...SECONDARY_CONSENT_CODE_INFO[
                                            sc.code
