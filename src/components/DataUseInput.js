            ...
            {SECONDARY_CONSENT_CODE_KEYS.map((scc) => (
              <List.Item key={scc}>
                <List.Item.Meta
                  title={
                    <Checkbox
                      checked={this.state.consent_code.secondary_categories.map((c) => c.code).includes(scc)}
                      onChange={(e) => this.handleSCCChange(e, scc)}
                    >
                      {SECONDARY_CONSENT_CODE_INFO[scc].title}
                    </Checkbox>
                  }
                  description={<div style={{ marginLeft: "24px" }}>{SECONDARY_CONSENT_CODE_INFO[scc].content}</div>}
                />
              </List.Item>
            ))}
          </List>
              ...
              return (
                <List.Item>
                  <List.Item.Meta
                    ...
                    title={
                      <Checkbox
                        checked={this.state.data_use_requirements.map((c) => c.code).includes(u)}
