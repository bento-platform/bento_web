...
const ReferenceGenomesContent = () => {
  ...
  const referenceIngestionWorkflows = useMemo(
    () => ingestionWorkflows.filter((w) => (w.tags ?? []).includes("reference")),
    [ingestionWorkflows],
  );

  // Fallback: if our chosen default isn't available, use the first available reference-tagged ingestion workflow.
  const defaultIngestionWorkflow =
    ingestionWorkflowsByID[DEFAULT_REF_INGEST_WORKFLOW_ID] ?? referenceIngestionWorkflows[0];
    ...
    () => [
      ...
      },
      {
        ...
        render: (genome) => (
          <div>
            <strong>FASTA:</strong>&nbsp;
            <a target="_blank" rel="noopener noreferrer" href={genome.fasta}>
              {genome.fasta}
            </a>
            <br />
            <strong>FAI:</strong>&nbsp;
            <a target="_blank" rel="noopener noreferrer" href={genome.fai}>
              {genome.fai}
            </a>
            {genome.gff3_gz && (
              <>
        ...
        ? [
            {
              ...
              render: (genome) => (
                <Space>
                  <Popover
                    title={`Reference Genome: ${genome.id}`}
                    content={
                      <Descriptions layout="horizontal" bordered={true} column={1} size="small">
                        <Descriptions.Item label="Checksums">
                          <strong>MD5:</strong>&nbsp;<MonospaceText>{genome.md5}</MonospaceText>
                          <br />
                          <strong>GA4GH:</strong>&nbsp;
                          <MonospaceText>{genome.ga4gh}</MonospaceText>
                        </Descriptions.Item>
                      </Descriptions>
                    }
                  >
