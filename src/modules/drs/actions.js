...
export const performDownloadFromDrsIfPossible = (filename) => async (dispatch, getState) => {
        ...
        let fuzzySearchUrl = `${getState().services.itemsByArtifact.drs.url}/search?fuzzy_name=${filename}`;
        ...
        let fuzzySearchObj = getState().drs?.fuzzySearchResponse;
        ...
        let objId = fuzzySearchObj.find(obj => obj.name == filename)?.id;
        if (fuzzySearchObj == undefined) {
        ...
        let downloadUrl = `${getState().services.itemsByArtifact.drs.url}/objects/${objId}/download`;
